...
Afridock is a **multi-tenant SaaS + self-hostable middleware layer** sitting between African enterprises and a pool of open-source/commercial LLMs, exposing a unified chat/API surface. Five architectural principles drive most design decisions and should be checked against when implementing any feature — the first outranks the rest when they conflict:

- **Low-cost AI via open-source models is the #1 constraint.** Every model-routing, infra, and pricing decision defaults to free/self-hosted open-weight inference (Llama/Mistral/Mixtral via Ollama/vLLM/HF); commercial providers (Claude/OpenAI) are an explicit, cost-flagged, org-level opt-in escape hatch, never a silent default. Concretely: `inference/fallback.py`'s `FallbackChain` never routes to a commercial (`is_commercial: true`) profile unless the organization has set `allow_commercial_fallback = true` — see `progress.md`'s decisions log for the enforcement details. This is a business-model requirement for price-sensitive African markets, not a stylistic preference — treat it as a hard constraint when adding models, providers, or pricing logic, not a tiebreaker.
- **Provider-agnostic inference**: all model calls go through an inference orchestrator (LiteLLM) that abstracts provider/model/transport, so switching between Hugging Face-hosted Llama, self-hosted vLLM/Ollama, or a commercial fallback (Claude/OpenAI) requires no application code changes. Default model is Llama 3.1 8B/70B Instruct (local/self-hosted dev runs a real self-hosted Ollama model, `llama-3.2-1b-instruct`, ahead of it in the fallback chain).
